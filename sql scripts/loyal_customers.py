...
def generate_sql(data):
    ...
    sql = "INSERT INTO loyal_customers (dealership_id_id, customer_id_id, loyalty_points, registration_year) VALUES "
    i = 0
    file_index = 0
    for entity in data:
        i += 1
        ...
        if i % (ROWS_PER_BATCH * DATA_FOR_ROW_TO_GENERATE) == 0:
            with open(f"loyal_customers{file_index}.sql", "a") as file:
                file.write(sql[:-2] + ";")
    ...
    if sql != "INSERT INTO loyal_customers (dealership_id_id, customer_id_id, loyalty_points, registration_year) VALUES ":
